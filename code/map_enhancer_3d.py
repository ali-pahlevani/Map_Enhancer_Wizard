...
class MapEnhancer3D(tk.Tk):
    ...
    `pose_z`. Finally, if a table contains a binary `pose` blob (as used by
    RTAB-Map's `Node` table), the translation portion of that blob is decoded
    to obtain coordinates. The detected data are used to build a minimal point
    cloud which can then be filtered by the threshold slider.
    ...
    def load_map(self, file_path):
        try:
            ...
            table, cols, mode = self._detect_point_table(cursor)
            if table is None:
            ...
            if mode == "columns":
            ...
            else:  # blob mode (e.g., RTAB-Map 'pose' column)
                ...
                for (blob,) in cursor.fetchall():
                    if blob is None:
                        continue
                    try:
                        # pose stored as 7 floats (x, y, z, qx, qy, qz, qw)
                        x, y, z = struct.unpack('fff', blob[:12])
    ...
    def _detect_point_table(self, cursor):
        """Return `(table, cols, mode)` describing how to extract XYZ points.

        - For regular tables with numeric columns, `mode` is ``"columns"`` and
          `cols` is a 4-tuple of column names (the fourth may be ``None``).
        - For tables containing a binary ``pose`` column (as in RTAB-Map),
          `mode` is ``"blob"`` and `cols` is a single-element tuple containing
        ...
        for table in tables:
            ...
            if {'x', 'y', 'z', 'value'} <= colset:
                return table, ('x', 'y', 'z', 'value'), 'columns'
            if {'x', 'y', 'z'} <= colset:
                return table, ('x', 'y', 'z', None), 'columns'
            if {'pose_x', 'pose_y', 'pose_z'} <= colset:
                return table, ('pose_x', 'pose_y', 'pose_z', None), 'columns'
            if 'pose' in colset:
                return table, ('pose',), 'blob'
